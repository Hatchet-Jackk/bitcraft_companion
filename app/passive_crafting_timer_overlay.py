...
class PassiveCraftingTimerOverlay(ctk.CTkToplevel):
    ...
    Provides real-time display, filtering, and sorting of passive crafting operations
    for the current user, including auto-refresh and expandable row details.
    """
    ...
    def __init__(
        ...
        """Initialize the PassiveCraftingTimerOverlay window.

        Args:
            parent: Parent window instance
            bitcraft_client: BitCraft client for API/database access
            passive_crafting_service: Service for passive crafting data
        """
    ...
    def on_data_received(self, data: List[Dict[str, Any]], success: bool, message: str, has_data: bool):
        ...
        Args:
            data: List of timer data rows
            success: True if data fetch succeeded, False otherwise
            message: Status message for UI
            has_data: True if data is present, False otherwise
        """
        if success:
    ...
    def save_expansion_state(self):
        """Save the current expansion state of parent items in the treeview."""
    ...
    def restore_expansion_state(self):
        """Restore the expansion state of parent items in the treeview."""
        # Iterate through all top-level items (parents)
        for parent_id in self.tree.get_children():
            if self.tree.get_children(parent_id):  # Has children (is a parent)
                # Create a unique key for this parent based on its values
    ...
    def populate_tree(self, data: List[Dict[str, Any]]):
        ...
        Args:
            data: List of timer data rows to display
        """
    ...
    def apply_filters_and_sort(self):
        """Apply search filters and sorting to the timer data and update the treeview."""
    ...
    def _show_combined_menu(self, column):
        """Show a combined sort/filter menu for the given column."""
        ...
        menu.add_command(
            ...
            command=lambda: self._show_filter_dialog(column),
        )

        if self._is_filter_active(column):
            menu.add_command(
                label=f"Clear {column} Filter",
                command=lambda: self._clear_column_filter(column),
            )
    ...
    def _show_filter_dialog(self, column_name: str):
        """Show a filter dialog for the specified column."""
    ...
    def on_double_click(self, event):
        """Handle double-click to expand or collapse parent rows in the treeview."""
    ...
    def on_closing(self):
        """Handle window closing (called by the parent window's protocol)."""
