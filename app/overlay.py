...
class BitCraftOverlay(BaseWindow):
    """Overlay window for BitCraft login and main navigation.

    Handles login, authentication, player name entry, and navigation to main
    application toggles. Manages UI state transitions and credential storage.
    """
    ...
    def setup_ui(self):
        """Setup the UI for the overlay window.

        Note: Actual UI setup is handled by state transitions in this overlay.
        """
        pass

    def _transition_to_ui_state(self, new_state: OverlayState):
        """Transition to a new UI state, clearing and setting up the appropriate UI.

        Args:
            new_state: The OverlayState to transition to
        """
    ...
    def _setup_access_code_ui(self):
        """Set up the UI for access code input during authentication flow."""
    ...
    def _setup_player_name_ui(self):
        """Set up the UI for player name input and region selection."""
        ...
        if has_credentials:
            ...
            email_label = ctk.CTkLabel(
                self.player_name_frame,
                text=f"Email: {self.stored_email}",
                text_color="gray",
            )
    ...
    def _setup_main_toggles_ui(self):
        """Set up the UI with application toggles and navigation buttons."""
    ...
    def _back_to_login(self):
        """Go back to the login screen and reset toggle/window state."""
    ...
    def get_connection_data(self):
        """Return the connection data if successful.

        Returns:
            dict: Dictionary with email, player_name, and bitcraft_client
        """
        return {
    ...
    def toggle_claim_inventory_window(self):
        """Toggle the claim inventory window on or off."""
        # Use the base class implementation
        super().toggle_claim_inventory_window()

    def _get_access_code_flow(self):
        """Handle getting the access code from the API for the entered email."""
    ...
    def _run_get_access_code(self, email: str):
        """Run the get access code request in a separate thread.

        Args:
            email: Email address to request access code for
        """
        try:
    ...
    def _authenticate_flow(self):
        """Handle authentication with the access code entered by the user."""
    ...
    def _run_authenticate(self, email: str, access_code: str):
        """Run the authentication request in a separate thread.
    ...
    def _connect_flow(self):
        """Handle the final connection step with player name and region."""
    ...
    def _run_connect(self, player_name: str, region: str):
        ...
        Args:
            player_name: Player name to connect with
    ...
    def _logout_flow(self):
        """Handle logout from the overlay and clear credentials."""
