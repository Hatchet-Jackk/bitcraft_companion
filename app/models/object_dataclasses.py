...
class ClaimLocalState:
    ...
    def from_array(cls, data: List) -> "ClaimLocalState":
        ...
        if not isinstance(data, list) or len(data) < 11:
            raise ValueError(
                f"Invalid claim_local_state array format: expected 11+ elements, got {len(data) if hasattr(data, '__len__') else 'unknown'}"
            )

        return cls(
            entity_id=data[0],
            supplies=data[1],
            building_maintenance=data[2],
            num_tiles=data[3],
            num_tile_neighbors=data[4],
            location=data[5],
            treasury=data[6],
            xp_gained_since_last_coin_minting=data[7],
            supplies_purchase_threshold=data[8],
            supplies_purchase_price=data[9],
            building_description_id=data[10],
        )
