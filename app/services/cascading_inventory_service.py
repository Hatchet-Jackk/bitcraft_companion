...
class CascadingInventoryCalculator:
    ...
    def apply_cascading_reductions(
        ...
        dependency_trees: Dict[str, Dict]
    ) -> Dict[str, Dict]:
        """
        ...
        inventory_items = self._sort_inventory_by_complexity(inventory, dependency_trees)
    ...
    def _sort_inventory_by_complexity(
        self, 
        inventory: Dict[str, int], 
        dependency_trees: Dict[str, Dict]
    ) -> List[Tuple[str, int]]:
        """
        Sort inventory items by complexity (dependency count) for optimal reduction.
        
        Items with more dependencies are processed first to maximize reductions.
        """
        ...
        for material_name, quantity in inventory.items():
            ...
            material_info = dependency_trees.get(material_name, {})
            dependencies = material_info.get('dependencies', {})
            dep_count = len(dependencies.get('all', []))
            
            inventory_with_complexity.append((material_name, quantity, dep_count))
        
        # Sort by dependency count (descending) then by quantity (descending)
    ...
    for profession, tiers in profession_templates.items():
        for tier, materials in tiers.items():
            for material_name, material_info in materials.items():
                ...
                elif len(material_info.get('dependencies', {}).get('all', [])) > len(dependency_trees[material_name].get('dependencies', {}).get('all', [])):
                    dependency_trees[material_name] = material_info
