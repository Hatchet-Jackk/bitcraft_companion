...
class TestDataServiceInitialization:
    ...
    def test_double_start_prevention(self):
        ...
        with patch.object(data_service, '_run', side_effect=mock_run_keeps_alive), \
...
class TestDataServiceAuthentication:
    ...
    def test_successful_authentication_flow(self):
        ...
        with patch('app.services.inventory_service.InventoryService'), \
             patch('app.services.passive_crafting_service.PassiveCraftingService'), \
             patch('app.services.traveler_tasks_service.TravelerTasksService'), \
             patch('app.services.active_crafting_service.ActiveCraftingService'), \
             patch('app.services.claim_service.ClaimService') as mock_claim_service:
...
class TestClaimSwitching:
    ...
    def test_claim_switching_failure(self):
        ...
        any_errors = any(msg.get("type") == "claim_switched" and 
                        msg.get("data", {}).get("status") == "error" 
                        for msg in messages)
